...
| Backups asked for **during setup** (PRD I-6): the wizard's step 4 of 5, with "Skip for now" allowed, and the Overview nags until a folder is set and a backup is recent | Done |
| **Packaged Windows build verified** (first time since M1): the keychain is reachable from inside the PyInstaller bundle, a backup was written by the packaged app, the updates endpoint answers, 8 sample reports read, and the **tray icon was exercised** with no console fallback | Done |
| First outside testers | **Going ahead unsigned** (owner's call, 11 Sep): [INSTALL.md](docs/desktop/INSTALL.md) explains the "Windows protected your PC" warning and asks testers to report whether it put them off. Signing stays the fix before any wider release |
